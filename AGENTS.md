...
- 不插入外部热链图片。
- 不随意改图片文件名，避免破坏已有引用。
- 新增犬种图片必须优先使用高清、写实、生动的真实摄影风格，不使用低清插画、模糊图、变形犬只或明显 AI 伪影。
- 生成图片必须符合犬种真实外观特征，例如毛色、耳型、体型、被毛质感、面部比例和典型姿态。
- 所有生成图片必须无文字、无 logo、无水印、无签名、无品牌项圈文字。
- `cover` 建议使用竖图或适合首页卡片裁切的构图，当前标准尺寸为 `1600x1885`。
- `main` 建议使用横图，当前标准尺寸为 `1920x1080`，用于正文首图展示犬种典型外观。
- `play` 建议使用横图或轻微动态构图，当前标准尺寸为 `1600x1200`，用于展示户外运动、互动或性格。
- 批量生成图片前，优先参考项目根目录的 `image-generation-prompts.json`，保持输出文件名不变，避免破坏文章引用和 Google 已索引图片路径。
